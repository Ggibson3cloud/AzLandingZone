...
resource "azurerm_virtual_network_peering" "hub_to_identity" {
  ...
  name                         = "hub_vnet_to_stage_azure_sql_vnet"
  resource_group_name          = data.azurerm_resource_group.hub_rg.name
  virtual_network_name         = data.azurerm_virtual_network.hub_vnet.name
  remote_virtual_network_id    = module.vnet["s-eus2-vnet-sql"].id ##update format "i-eus2-vnet-identity"
...
resource "azurerm_virtual_network_peering" "identity_to_hub" {
  name                         = "stage_azure_sql_vnet_to_hub_vnet"
  resource_group_name          = module.resource_group.name
  virtual_network_name         = module.vnet["s-eus2-vnet-sql"].vnet_name ##update format "i-eus2-vnet-identity"
