...
/*resource "azurerm_subnet_route_table_association" "this" {
  for_each       = { for key, value in var.subnet : key => value if lookup(value, "route_table", null) != null }
  subnet_id      = module.identity_subnets[each.key].subnets_id
  route_table_id = module.route_tables["${each.key}-rt"].route_table_id
}*/
...
resource "azurerm_virtual_network_peering" "hub_to_identity" {
  ...
  name                         = "hub_vnet_to_development_apps_vnet"
  resource_group_name          = data.azurerm_resource_group.hub_rg.name
  virtual_network_name         = data.azurerm_virtual_network.hub_vnet.name
  remote_virtual_network_id    = module.vnet["d-cus-vnet-apps"].id ##update format "i-cus-vnet-identity"
...
resource "azurerm_virtual_network_peering" "identity_to_hub" {
  name                         = "development_apps_vnet_to_hub_vnet"
  resource_group_name          = module.resource_group.name
  virtual_network_name         = module.vnet["d-cus-vnet-apps"].vnet_name ##update format "i-cus-vnet-identity"
