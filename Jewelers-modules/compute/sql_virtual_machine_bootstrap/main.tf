...
resource "azurerm_key_vault_secret" "randpass" {
  name         = join("-", [var.sql_admin_user, var.randpass])
...
# Commented out due to Extension not being fully provisioned on the VM due to issues with the SQLServerConfig.ps1 script which caused the data disks to get reformatted after every reboot
# locals {
